...
void IntervalCounterImp::SetIntervals(const string &intervalSrcFileName)
{
	...
	BigTextInputFile file(intervalSrcFileName);
	string text;
	file.ReadAll(text);

	// parse the result
	DataParser::ParseIntervalStringToNumber(text, m_LowerBounds, m_UpperBounds);
...
struct SweepLineFunctor{
	SweepLineFunctor(const vector<unsigned int> &lowerBounds,
					 const vector<unsigned int> &upperBounds):
	m_SortedLowerBounds(const_cast<vector<unsigned int> &> (lowerBounds)),
	m_SortedUpperBounds(const_cast<vector<unsigned int> &> (upperBounds)){

	}

	unsigned int operator()(unsigned int val){
		// find out how many "lowerBounds" and "upperBounds" val has sweeped
		vector<unsigned int>::iterator sweepCountInLowerBound = 
				upper_bound(m_SortedLowerBounds.begin(), m_SortedLowerBounds.end(), val);
		vector<unsigned int>::iterator sweepCountInUpperBound = 
	...
	vector<unsigned int> &m_SortedLowerBounds;
	vector<unsigned int> &m_SortedUpperBounds;
};
...
using namespace BigTest::Intervals;
